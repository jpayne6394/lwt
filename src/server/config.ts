export type RuntimeConfig = {
  ...
  localDevMemorySeedEnabled: boolean;
  superAgentMemorySeedsPath: string;
};
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): RuntimeConfig {
  return {
    ...
    localDevMemorySeedEnabled: parseBoolean(
      env.LOCAL_DEV_MEMORY_SEED_ENABLED,
      !env.DATABASE_URL && env.RENDER !== "true" && env.NODE_ENV !== "production",
    ),
    superAgentMemorySeedsPath: env.SUPER_AGENT_MEMORY_SEEDS_PATH ?? "config/super-agent-memory-seeds.json",
  };
}
