...
test("content radar turns curated social listening seeds into blog structure evidence", async () => {
  const repository = new MemoryRepository();
  ...
  const result = await runContentRadarAgent({
    ...
    radarSettings: {
      ...
      xQueries: [],
      searchQueries: [],
      scanFrequencyNotes: "Manual listening seed test.",
      listeningSeeds: [
        {
          topic: "magnesium",
          priority: "high",
          audienceQuestion: "Which magnesium form should I take if glycinate makes me groggy?",
          reactionThemes: ["People compare forms by morning feel, stomach comfort, and sleep routine fit."],
          objectionThemes: ["Worried about stomach upset and choosing the wrong form."],
          personalExperienceAngles: ["Tried citrate first, then switched to glycinate for evening use."],
          popularStructurePatterns: ["Question hook -> form comparison -> personal routine checklist -> practitioner CTA"],
          safeBlogAngles: ["Magnesium forms explained through real routine questions"],
          relatedProducts: ["Magnesium Glycinate"],
          relatedCollections: ["Magnesium", "Sleep Support"],
        },
      ],
    },
  });

  const listeningSource = result.sourceItems.find((item) => item.scoreJson.source === "curated_social_listening_seed");
  assert(listeningSource);
  assert.equal(listeningSource.source, "manual");
  assert.match(listeningSource.title, /magnesium/i);
  assert.match(listeningSource.textExcerpt, /Which magnesium form should I take/);
  assert.match(listeningSource.textExcerpt, /Question hook/);
  assert.deepEqual(listeningSource.scoreJson.popularStructurePatterns, [
    "Question hook -> form comparison -> personal routine checklist -> practitioner CTA",
  ]);

  const idea = result.ideas.find((candidate) => candidate.topic === "magnesium");
  assert(idea);
  assert.match(idea.sourceSummary, /curated social listening/i);
  assert.match(idea.sourceSummary, /Which magnesium form should I take/);
  assert.match(idea.sourceSummary, /Question hook/);
  assert.match(idea.suggestedTitle, /Magnesium Forms Explained Through Real Routine Questions/);
});
